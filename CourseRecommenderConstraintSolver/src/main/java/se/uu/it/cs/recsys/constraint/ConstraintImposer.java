...
class ConstraintImposer {
    ...
    public static boolean postTotalCreditsConst(Store store, SetVar allIdUnion, Map<CourseCredit, Set<Integer>> creditToInterestedCourseIds) {
        ...
        store.impose(new XgteqC(totalCredits, Solver.MIN_ALL_PERIODS_CREDIT * Solver.CREDIT_NORMALIZATION_SCALE));
        store.impose(new XlteqC(totalCredits, Solver.MAX_ALL_PERIODS_CREDIT * Solver.CREDIT_NORMALIZATION_SCALE));
    ...
    public static boolean postMustHaveElemInSetConst(Store store, SetVar[] periodVars, Map<Integer, Set<Integer>> periodToMustHaveElem) {
        List<Boolean> unsatisfied = new ArrayList<>();

        periodToMustHaveElem.entrySet().forEach(period -> {
            SetVar periodVar = periodVars[period.getKey() - 1];

            period.getValue().forEach(courseId -> {
                store.impose(new EinA(courseId, periodVar));
                if (!store.consistency()) {
                    unsatisfied.add(Boolean.FALSE);
                }
            });
        });
    ...
    public static boolean postEachPeriodCreditConst(Store store, SetVar[] periodVars, Map<CourseCredit, Set<Integer>> creditToInterestedCourseIds) {
        ...
        for (SetVar singlePeriodVar : periodVars) {
            ...
            LOGGER.debug("After posting single period credits constraint on {}, is store still consistent?{}", singlePeriodVar.id(), store.consistency());
        }
