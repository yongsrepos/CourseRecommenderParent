...
public class CourseDomainRelevanceRepositoryIT {
    
    private final static Logger LOGGER = LoggerFactory.getLogger(CourseDomainRelevanceRepositoryIT.class);
    
    @Autowired
    private CourseDomainRelevanceRepository courseDomainRelevanceRepository;
    
    public CourseDomainRelevanceRepositoryIT() {
    }
    
    @Test
    public void testFindByDomainId() {
        
        Set<CourseDomainRelevance> relSet = this.courseDomainRelevanceRepository.findByDomainId("10010382");
        
        LOGGER.info("Test output {}", relSet);
